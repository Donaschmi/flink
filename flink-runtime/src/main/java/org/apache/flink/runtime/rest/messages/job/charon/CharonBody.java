 ...
 */
package org.apache.flink.runtime.rest.messages.job.charon;
...
@FlinkJsonSchema.AdditionalFields(type = Map.class)
public class CharonBody implements RequestBody, ResponseBody {

    @JsonAnySetter
    @JsonAnyGetter
    @JsonSerialize(keyUsing = JobVertexIDKeySerializer.class)
    @JsonDeserialize(keyUsing = JobVertexIDKeyDeserializer.class)
    private final Map<JobVertexID, JustinVertexResourceRequirements> charon;
    public CharonBody() {this(null);}
    public CharonBody(
            @Nullable JustinResourceRequirements jobResourceRequirements) {
        if (jobResourceRequirements != null) {
            this.charon = jobResourceRequirements.getJobVertexParallelisms();
        } else {
            this.charon = new HashMap<>();
        }
    }
    @JsonIgnore
    public Optional<JustinResourceRequirements> asCharonResourceRequirements() {
        if (charon.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(new JustinResourceRequirements(charon));
    }
    @Override
    public boolean equals(Object o) {
        ...
        final CharonBody that = (CharonBody) o;
        return Objects.equals(charon, that.charon);
    }
    @Override
    public int hashCode() {
        return Objects.hash(charon);
    }
}
